...
revert_repo_to_february() {
  ...
  echo "Searching for commits in February 2023 in $repo_path..."

  # Find the last commit made in February
  commit_hash=$(git rev-list -1 --before="2023-03-01 00:00" --after="2023-01-31 23:59" HEAD)

  # Check if a valid commit hash was found
  if [ -z "$commit_hash" ]; then
    echo "No commits found in February 2023 in $repo_path"
  else
    # Reset the repository to that commit
    git reset --hard "$commit_hash" || { echo "Error: Failed to reset repository $repo_path"; return 1; }
    echo "Reverted $repo_path to the last commit in February 2023: $commit_hash"
  fi
