...
start_reproxy() {
    ...
    # Truncate the reproxy log at the start of each run. Otherwise it just
    # accumulates forever and the trailing `cat` at the end of this script
    # mixes stale errors from old runs in with the current one, making it
    # look like problems are still happening when they've already been fixed.
    : > "${RBE_REPROXY_LOG}"
